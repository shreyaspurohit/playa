...
describe('cloud sync document', () => {
  ...
  test('a fresh device creates adds but never infers deletes', () => {
    ...
    const doc = localToSyncDoc(localStorage, ['directory'], null, 'device-a', 200);
    assert.equal(doc.sets['favs/directory']['missing'], undefined);
    assert.deepEqual(JSON.parse(JSON.stringify(doc.sets['favs/directory'])), { local: { t: 200 } });
  });

  test('a known baseline turns a removed favorite into a tombstone', () => {
    const baseline = emptySyncDoc('device-a', 100);
    baseline.sets['favs/directory'] = { keep: { t: 100 }, remove: { t: 100 } };
    localStorage.setItem(scopedKey(LS.favs, 'directory'), JSON.stringify(['keep']));
    const next = localToSyncDoc(localStorage, ['directory'], baseline, 'device-a', 200);
    assert.deepEqual(next.sets['favs/directory'].keep, { t: 100 });
    assert.deepEqual(next.sets['favs/directory'].remove, { t: 200, del: 1 });
  });

  test('an unchanged local snapshot preserves baseline metadata', () => {
    const baseline = emptySyncDoc('device-a', 100);
    baseline.sets['favs/directory'] = { keep: { t: 100 } };
    localStorage.setItem(scopedKey(LS.favs, 'directory'), JSON.stringify(['keep']));
    assert.deepEqual(
      JSON.parse(JSON.stringify(localToSyncDoc(
        localStorage, ['directory'], baseline, 'device-b', 200,
      ))),
  ...
  test('merge is commutative, idempotent, and active wins an exact tie', () => {
    ...
    a.sets['favs/directory'] = { x: { t: 20, del: 1 } };
    b.sets['favs/directory'] = { x: { t: 20 }, y: { t: 19 } };
    ...
    assert.deepEqual(ab.sets['favs/directory'].x, { t: 20 });
  ...
  test('meet spots merge per item and a known-baseline removal becomes a tombstone', () => {
    ...
    aStorage.setItem(scopedKey(LS.meetSpots, 'directory'), JSON.stringify([
      { label: 'A', address: '6:00 & A' },
    ]));
    const a = localToSyncDoc(aStorage, ['directory'], null, 'a', 10);
    aStorage.setItem(scopedKey(LS.meetSpots, 'directory'), JSON.stringify([
      { label: 'B', address: '7:00 & B' },
    ]));
    const b = localToSyncDoc(aStorage, ['directory'], null, 'b', 20);
    const merged = mergeSyncDocs(a, b, 21);
    applySyncDoc(aStorage, merged);
    assert.deepEqual(
      JSON.parse(aStorage.getItem(scopedKey(LS.meetSpots, 'directory'))!).map((s: { label: string }) => s.label).sort(),
      ['A', 'B'],
    );
    ...
    const removed = localToSyncDoc(aStorage, ['directory'], merged, 'a', 30);
    const entries = Object.entries(removed.registers)
      .filter(([key]) => key.startsWith('meetSpot/directory/'));
  ...
  test('old tombstones are garbage collected', () => {
    ...
    doc.sets['favs/directory'] = { old: { t: 1, del: 1 }, live: { t: 1 } };
    const out = mergeSyncDocs(doc, emptySyncDoc('b', 2), TOMBSTONE_MAX_AGE_MS + 2);
    assert.equal(out.sets['favs/directory'].old, undefined);
    assert.deepEqual(out.sets['favs/directory'].live, { t: 1 });
  });

  test('apply restores every synced field across sources', () => {
    const doc = emptySyncDoc('cloud', 10);
    doc.sets['favs/directory'] = { c1: { t: 10 }, c2: { t: 9, del: 1 } };
    doc.sets['favEvents/directory'] = { e1: { t: 10 } };
    doc.sets['favArt/api-2026'] = { a1: { t: 10 } };
    doc.sets['hiddenDays/directory'] = { 'e1|2026-08-31': { t: 10 } };
    ...
    assert.equal(localStorage.getItem(scopedKey(LS.myCampId, 'directory')), 'c1');
    assert.equal(JSON.parse(localStorage.getItem(scopedKey(LS.sharedFavs, 'directory'))!).Ada.name, 'Ada');
  ...
  test('parser rejects oversized, wrong-schema, prototype and malformed entries', () => {
    ...
    good.sets['favs/directory'] = { ok: { t: 10 } };
    assert.deepEqual(
    ...
    const poisoned = JSON.stringify({
      ...good,
      sets: JSON.parse('{"favs/directory":{"__proto__":{"t":10}}}'),
    });
    assert.equal(parseSyncDoc(poisoned), null);
    const malformed = { ...good, sets: { 'favs/directory': { ok: { t: 'late' } } } };
